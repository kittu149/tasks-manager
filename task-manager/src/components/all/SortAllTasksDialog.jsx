...
export default function SortAllTasksDialog({ open, onClose, tasks = [] }) {
  const [list, setList] = useState([]);

  useEffect(() => { setList(tasks || []); }, [tasks]);

  function applyRecommended() { setList(sortTasks(list)); }
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth>
      <DialogTitle>Sort All Tasks</DialogTitle>
      <DialogContent>
        <Button variant="outlined" onClick={applyRecommended} sx={{ mb: 2 }}>Apply Recommended</Button>
        <List>
          {list.map((t,i) => <ListItem key={t.id} divider><ListItemText primary={`${i+1}. ${t.title}`} secondary={`Score: ${t._score?.toFixed ? t._score.toFixed(2) : "-"}`} /></ListItem>)}
        </List>
      </DialogContent>
      <DialogActions>
        <Button onClick={onClose}>Cancel</Button>
        <Button variant="contained" onClick={save}>Save Order</Button>
      </DialogActions>
    </Dialog>
  );
}
